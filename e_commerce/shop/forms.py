...
from .models import Buyer, Order, Review, Sale
...
class CheckoutForm(forms.Form):
    ...
    order = forms.ModelChoiceField(
        required=False, queryset=Order.objects.all(), widget=forms.HiddenInput()
    )
...
class BrandFilterForm(forms.Form):
    def __init__(self, choices, *args, **kwargs):
        ...
        self.fields["brand"] = forms.MultipleChoiceField(
            choices=choices, widget=forms.CheckboxSelectMultiple, label="Бренд"
        )
